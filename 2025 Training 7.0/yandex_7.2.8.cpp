...
using namespace std;
int count_max(vector<pair<int,int>>&,int,int ,int,int,int);
void add_t(vector<pair<int,int>>&,int,int ,int,int,int,int);
void push_t(vector<pair<int,int>>& tree_seg,int v,int l,int r);
int main()
{
    int N,K;
    cin>>N;
    int P = 1;
    while(P<N) P*=2;
    vector<pair<int,int>> tree_seg(P*2-1,{-1,0});
    for(int i =0 ;i<N;i++)
    {
    ...
    for(int i = 0;i<K;i++)
    {
        ...
        {
            cin>>L>>R;
            cout<<count_max(tree_seg,0,0,P-1,L-1,R-1)<<" ";
        }
        else{
            cin>>L>>R>>X;
...
int count_max(vector<pair<int,int>>& tree_seg,int v,int l,int r,int L,int R)
{
...
void add_t(vector<pair<int,int>>& tree_seg,int v,int l,int r,int L,int R,int x)
{
    push_t(tree_seg,v,l,r);
    if(R<l || L>r) return;
    if(L<=l  && r<=R)
